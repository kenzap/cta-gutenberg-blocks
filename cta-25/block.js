...
export const defaultItems = JSON.stringify( [
    {
        ...
        // colorIcon: "#333",
        // colorText: "#333",
        // colorIconHover: "#0047ff",
        // colorTextHover: "#0047ff",
...
export const onTabClick = ({target:tab}, index) => {
    ...
    //$('.kenzap .kp-tabs-1 .kp-nav ul li').removeClass('active');
    $(tab).parent().parent().find('li').removeClass('active');
    $(tab).parent().addClass('active');
    $(tab).parent().parent().parent().parent().parent().find('.tab-content').hide();
    //$('.kenzap .kp-tabs-1 .tab-content').hide();
...
registerBlockType( 'kenzap/cta-25', {
    title: __( 'Kenzap Tabs 1', 'kenzap-cta' ),
    icon: 'migrate',
    category: 'layout',
    keywords: [
        __( 'cta 25', 'kenzap-cta' ),
                        ...
                        <div class={ `kp-nav ${ attributes.tabStyle == "button" ? "kp-button":"" }` }>
                            <div class="kp-nav-inner" >
                                <ul class="nav-tabs">
                                    ...
                                    { attributes.items && attributes.items.map( ( item, index ) => (
                                    <li data-index={index} className={ `${ index == 0 ? 'kptab active':'kptab' }` } style={ {...getTypography( attributes, 0, 'color' ), ...getTypography( attributes, 0, '--hcl' ), ...getTypography( attributes, 0, '--hbacl' ), ...getTypography( attributes, 0, '--hbrcl' )} }>

            ...
            <div id={ `content-${ index }` } class="tab-content" style={ { display: (`${ index == 0 ? 'block':'none' }`) } }>
                ...
                >
                    ...
                    <div class="kp-pc">
                        ...
                        />
                        {/* <p style={ {'margin-top':'5px' } }>{ __( 'Note. Other settings available in the right pane.', 'kenzap-plugin' ) }</p>  */}
                    </div>
                </Popover>
                }
                    ...
                    >
                        ...
                        <PanelBody
                                ...
                                <Fragment>
                                    { ( item.icon !== 'none' ) ? (
                                        <Fragment>
                                            <Button
                                                isDefault
                                                //onClick={ () => { this.onChangePropertyItem( 'icon', window.kenzap_plugin_path + "images/facebook-icon.svg", index, true ); } }
                                                onClick={ () => { this.onChangePropertyItem( 'icon', "none", index, true ); } }
                                            >
                                            { __( 'Remove', 'kenzap-cta' ) }
                                            </Button>
                                            <img class="cartIconMeasure" onLoad={ (e) => this.cartIconMeasure(e, index) } style={{ margin: '0 0 0px 5px', height: '27px', width: 'auto', }} src={ item.icon } />
                            ...
                            {/* <PanelColorSettings
                                ...
                                colorSettings={ [
                                    {
                                        value: item.colorIcon,
                                        onChange: ( value ) => {this.onChangePropertyItem( 'colorIcon', (value?value:"#333"), index, true );},
                                        label: __( 'Icon', 'kenzap-cta' ),
                                    },
                                    {
                                        value: item.colorText,
                                        onChange: ( value ) => {this.onChangePropertyItem( 'colorText', (value?value:"#333"), index, true );},
                                        label: __( 'Text', 'kenzap-cta' ),
                                    },
                                    {
                                        value: item.colorIconHover,
                                        onChange: ( value ) => {this.onChangePropertyItem( 'colorIconHover', (value?value:"#333"), index, true );},
                                        label: __( 'Icon hover', 'kenzap-cta' ),
                                    },
                                    {
                                        value: item.colorTextHover,
                                        onChange: ( value ) => {this.onChangePropertyItem( 'colorTextHover', (value?value:"#333"), index, true );},
                                        label: __( 'Text hover', 'kenzap-cta' ),
                                    },
                                ] }
                            /> */}
                            ...
                            <div class={ `kp-nav ${ attributes.tabStyle == "button" ? "kp-button":"" }` }>
                                <div class="kp-nav-inner" >
                                    <ul class="nav-tabs">
                                        ...
                                        { attributes.items && attributes.items.map( ( item, index ) => (
                                        <li onClick={ ( e ) => onTabClick(e, index) } className={ `${ index == 0 ? 'active':'' }` } style={ {...getTypography( attributes, 0, 'color' ), ...getTypography( attributes, 0, '--hcl' ), ...getTypography( attributes, 0, '--hbacl' ), ...getTypography( attributes, 0, '--hbrcl' )} }>
